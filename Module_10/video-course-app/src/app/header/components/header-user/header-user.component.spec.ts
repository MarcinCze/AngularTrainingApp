...
describe('HeaderUserComponent', () => {
  ...
  it('should have proper user name', () => {
    expect(
      fixture.debugElement.query(By.css('.user-btn.left>button'))
        .nativeElement.textContent).toContain(`${referenceAppUser.firstName} ${referenceAppUser.lastName}`);
  });
  ...
  it('should emit onUserLinkClick once clicked', () => {
    const spy = spyOn(component, 'onUserLinkClick');
    fixture.debugElement.query(By.css('.user-btn.left>button')).triggerEventHandler('click', null);
    fixture.detectChanges();
    expect(spy).toHaveBeenCalled();
  });
});
